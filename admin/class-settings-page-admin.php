...
class CAF_Settings_Page_Admin {
	...
	public function addPluginAdminMenu() {
		...
		add_menu_page(  $this->plugin_name, 'Casino Affiliate', 'administrator', $this->plugin_name, array( $this, 'displayPluginAdminDashboard' ), 'dashicons-embed-photo', 26 );
	...
	public function registerAndBuildFields() {
		...
		add_settings_field(
			'popup_ad_setting',
			'Popup Ad (popup in a new tab on the first click)',
			array( $this, 'settings_page_render_settings_field' ),
			'settings_page_general_settings',
			'settings_page_general_section',
			$args2
		...
		$args3 = array (
							'type'      => 'input',
							'subtype'   => 'checkbox',
							'id'    => 'popover_ad_setting',
							'name'      => 'popover_ad_setting',
							'required' => 'true',
							'get_options_list' => '',
							'value_type'=>'normal',
							'wp_data' => 'option'
		...
		add_settings_field(
			'popover_ad_setting',
			'Popover Ad (popover after 5 seconds)',
			array( $this, 'settings_page_render_settings_field' ),
			'settings_page_general_settings',
			'settings_page_general_section',
			$args3
		...
		$args4 = array (
							'type'      => 'input',
							'subtype'   => 'checkbox',
							'id'    => 'banner_ad_setting',
							'name'      => 'banner_ad_setting',
							'required' => 'true',
							'get_options_list' => '',
							'value_type'=>'normal',
							'wp_data' => 'option'
	...
	public function settings_page_display_general_account() {
		echo ''; //<h3><strong>Enter ONLY THE NUMBERS for your affiliate ID here (ie. 63511)</strong></h3>';
	}
			...
			case 'input':
					...
					if($args['subtype'] != 'checkbox'){
							...
							if(isset($args['disabled'])){
									...
									$disabledHtml = $prependStart.'<input type="'.$args['subtype'].'" id="'.$args['id'].'_disabled" '.$step.' '.$max.' '.$min.' name="'.$args['name'].'_disabled" size="40" disabled value="' . esc_attr($value) . '" /><input type="hidden" id="'.$args['id'].'" '.$step.' '.$max.' '.$min.' name="'.$args['name'].'" size="40" value="' . esc_attr($value) . '" />'.$prependEnd;
									echo wp_kses_normalize_entities($disabledHtml);
							} else {
									$enabledHtml = $prependStart.'<input type="'.$args['subtype'].'" id="'.$args['id'].'" "'.$args['required'].'" '.$step.' '.$max.' '.$min.' name="'.$args['name'].'" size="40" value="' . esc_attr($value) . '" />'.$prependEnd;
									echo wp_kses_normalize_entities($enabledHtml);
							}
							/*<input required="required" '.$disabled.' type="number" step="any" id="'.$this->plugin_name.'_cost2" name="'.$this->plugin_name.'_cost2" value="' . esc_attr( $cost ) . '" size="25" /><input type="hidden" id="'.$this->plugin_name.'_cost" step="any" name="'.$this->plugin_name.'_cost" value="' . esc_attr( $cost ) . '" />*/

					} else {
							$checked = ($value) ? 'checked' : '';
							$checkedHtml = '<input type="'.$args['subtype'].'" id="'.$args['id'].'" "'.$args['required'].'" name="'.$args['name'].'" size="40" value="1" '.$checked.' />';
							echo wp_kses_normalize_entities($checkedHtml);
					}
					break;
			default:
					# code...
